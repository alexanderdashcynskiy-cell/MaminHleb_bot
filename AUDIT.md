...
**Итоговое число проблем:** 197 (CRM: 91 + Bot Backend: 55 + Mini App: 46 + Cross: 5)  
**Метод:** Полное чтение всех исходных файлов + внешний список проблем  
**Ветка:** `claude/audit-white-fox-dashboard-W4DAS`

> **Что изменилось в v2.0:** Добавлены ~122 проблемы из внешнего списка, не вошедшие в первую версию аудита. Главный пропуск v1.0 — полностью отсутствовал анализ Mini App HTML файлов (`index.html`, `bot/index.html`).
...
| 8 | `src/App.tsx` | 1967 | Режим редактирования товара вызывает `/api/inventory/update`, но сервер обновляет только `stock`, `image`, `isNew`, `isBestSeller`; **`name`, `price`, `weight`, `category` не сохраняются** — UI показывает «Товар обновлён» при частичном обновлении |
| 9 | `server.ts` | 341, 365, 368 | Заказам присваивается `id: i` (индекс строки), затем массив `.reverse()`; обновление статуса по `id` обращается **к другой строке Sheets** после реверса |
| 10 | `server.ts` | 367–368 | Комментарии получают `id: i` по всем строкам и обрезаются `slice(-10)`; `/api/comments/reply` использует `id` как индекс строки — ответ записывается **не в тот заказ** |
| 11 | `src/App.tsx` | 214 | `parseDate` вызывает `new Date(dateStr)` для строк вида `12.05.2026 10:30` — браузер возвращает `Invalid Date`; статистика, фильтры «сегодня» и графики обнуляются |
| 12 | `src/App.tsx` | 2476 | `const result = JSON.parse(response.text)` без `try/catch` — Gemini возвращает markdown ` ```json ... ``` `, что вызывает `SyntaxError` и падение вкладки |
...
| 16 | `server.ts` | 380 | Сортировка `new Date(a.time)` для строк из Google Sheets без нормализации — русские даты дают `NaN` |
| 17 | `server.ts` | 417 | `filter(o => new Date(o.time).toLocaleDateString(...))` без проверки `Invalid Date` — рост считается неверно |
...
*Аудит v2.0 — коммит `cf5e4907` (crm-maminhleb7), коммит `53e7a9b` (maminhleb_bot)*  
*Дата: 17 мая 2026*
